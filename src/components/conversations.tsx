...
import { ArrowDownToLine, CheckCheck, MessageCircle, Search, Send, Sparkles } from "lucide-react";
...
interface ConversationsProps {
  ...
  onSend: (buyerId: string, text: string) => void;
...
export function Conversations({ products, buyers, messages, selectedBuyerId, onSelectBuyer, onSend, onSimulateMessage }: ConversationsProps) {
  ...
  useEffect(() => { setDraft(""); }, [selectedBuyerId]);
  ...
  function sendMessage(event: FormEvent<HTMLFormElement>) {
    ...
    if (!selectedBuyer || !draft.trim()) return;
    onSend(selectedBuyer.id, draft.trim());
    setDraft("");
  }
    ...
    <section aria-label="Сообщения" className="space-y-5">
      <div className="flex flex-wrap items-center justify-between gap-4">
        <div>
          ...
          <p className="mt-1 text-sm text-slate-500">Договоритесь о деталях и помогите покупателю решиться.</p>
        </div>
        <button type="button" className="button button-secondary" onClick={onSimulateMessage}>
          <ArrowDownToLine size={17} /> Получить сообщение
        </button>
      </div>
        ...
        {selectedBuyer ? (
          <div className="flex min-h-[480px] min-w-0 flex-col md:min-h-0">
            <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-100 px-6 py-4">
              ...
              <span className="inline-flex items-center gap-1.5 rounded-md bg-slate-100 px-2 py-1 text-[10px] font-medium text-slate-500"><Sparkles size={12} /> Демо-диалог</span>
            </div>
            ...
            <div className="min-h-0 flex-1 space-y-4 overflow-y-auto bg-[#fbfcfe] px-4 py-6 sm:px-6" role="log" aria-label={`Переписка с ${selectedBuyer.name}`} aria-live="polite">
              <p className="mb-6 text-center text-[10px] text-slate-400">Демонстрационная переписка · сообщения сохраняются в этом браузере</p>
              {conversation.map((message) => <div key={message.id} className={`flex ${message.direction === "outgoing" ? "justify-end" : "justify-start"}`}><div className={`max-w-[88%] rounded-2xl px-4 py-3 sm:max-w-[75%] ${message.direction === "outgoing" ? "rounded-br-md bg-blue-600 text-white" : "rounded-bl-md border border-slate-200 bg-white text-slate-700"}`}><p className="text-sm leading-relaxed whitespace-pre-wrap break-words">{message.text}</p><div className={`mt-2 flex items-center justify-end gap-1.5 text-[10px] ${message.direction === "outgoing" ? "text-blue-200" : "text-slate-400"}`}><span>{messageTime(message.createdAt)}</span>{message.direction === "outgoing" && <CheckCheck size={12} aria-label="Сохранено в демо" />}</div></div></div>)}
              {conversation.length === 0 && <div className="flex h-40 flex-col items-center justify-center text-center"><MessageCircle className="mb-3 text-slate-300" size={32} /><p className="text-sm text-slate-500">Начните знакомство</p><p className="mt-1 text-xs text-slate-400">Напишите покупателю первое сообщение.</p></div>}
              <div ref={messageEnd} />
            </div>
            <form onSubmit={sendMessage} className="border-t border-slate-100 bg-white px-4 py-4 sm:px-6">
              <div className="flex items-center gap-3"><input className="input min-w-0 flex-1" value={draft} onChange={(event) => setDraft(event.target.value)} maxLength={2000} aria-label="Текст сообщения" placeholder="Напишите сообщение…" autoComplete="off" /><button type="submit" className="button button-primary shrink-0 px-3!" disabled={!draft.trim()} aria-label="Отправить сообщение"><Send size={18} /></button></div>
              <p className="mt-2 text-[10px] text-slate-400">Enter — отправить · отправка имитируется внутри CRM</p>
            </form>
          </div>
        ) : <div className="flex min-h-[420px] flex-col items-center justify-center px-8 text-center"><span className="mb-5 flex size-16 items-center justify-center rounded-2xl bg-blue-50 text-blue-500"><MessageCircle size={29} strokeWidth={1.5} /></span><h3 className="text-base font-semibold text-slate-800">Все разговоры — в одном месте</h3><p className="mt-2 max-w-xs text-sm leading-relaxed text-slate-500">Выберите диалог слева или получите новое демо-сообщение.</p></div>}
      </div>
    </section>
  );
}
