...
export function Buyers({ buyers, products, onAdd, onUpdate, onOpenChat }: BuyersProps) {
  ...
  function saveBuyer(event: FormEvent<HTMLFormElement>) {
    ...
    const buyer: Buyer = { id: editingBuyer?.id ?? `buyer-${crypto.randomUUID()}`, name, phone, productId: draft.productId, status: draft.status, note: draft.note.trim() };
    if (editingBuyer) onUpdate(buyer); else onAdd(buyer);
    setModalOpen(false);
  }
  ...
  return (
    <section aria-label="Покупатели" className="space-y-5">
        ...
        {visibleBuyers.map((buyer, index) => {
          ...
          return <article key={buyer.id} className="flex flex-col rounded-2xl border border-slate-200 bg-white p-5 transition-shadow hover:shadow-sm">
            ...
            <div className="mt-4 flex flex-wrap items-center justify-between gap-2 border-t border-slate-100 pt-4">
              <select className="select max-w-[145px] text-xs!" value={buyer.status} aria-label={`Статус покупателя ${buyer.name}`} onChange={(event) => onUpdate({ ...buyer, status: event.target.value as Buyer["status"] })}>{BUYER_STATUSES.map((status) => <option key={status.id} value={status.id}>{status.label}</option>)}</select>
              <button type="button" className="button button-ghost gap-1.5! px-2! text-xs!" onClick={() => onOpenChat(buyer.id)}><MessageCircle size={15} /> Написать <ArrowUpRight size={13} /></button>
            </div>
          </article>;
        })}
      </div>
      ...
      <dialog ref={dialogRef} className="modal-panel" aria-labelledby="buyer-dialog-title" onCancel={() => setModalOpen(false)} onClose={() => setModalOpen(false)} onClick={(event) => { if (event.target === event.currentTarget) setModalOpen(false); }}>
        <form onSubmit={saveBuyer} onClick={(event) => event.stopPropagation()}>
          <div className="modal-header"><div><h2 id="buyer-dialog-title" className="text-lg font-semibold text-slate-900">{editingBuyer ? "Карточка покупателя" : "Новый покупатель"}</h2><p className="mt-1 text-sm text-slate-500">Все детали будущей сделки.</p></div><button type="button" className="icon-button" onClick={() => setModalOpen(false)} aria-label="Закрыть карточку покупателя"><X size={20} /></button></div>
          ...
          <div className="modal-footer"><button type="button" className="button button-secondary" onClick={() => setModalOpen(false)}>Отмена</button><button type="submit" className="button button-primary">{editingBuyer ? "Сохранить изменения" : "Создать покупателя"}</button></div>
        </form>
      </dialog>
    </section>
  );
